...
export const useMailStats = () => {
  ...
  // Use category-based logic for consistency with chart and tabs
  // ValidMails logic: category === "DungHan"
  const validMails = mails.filter(
    (mail) => mail.category === "DungHan"
  );

  // ExpiredMails logic: category === "QuaHan"
  const expiredMails = mails.filter(
    (mail) => mail.category === "QuaHan"
  );
  ...
  // Debug logging
  console.log("📊 Mail Stats Debug (Category-Based Logic):", {
    total,
    valid,
    expired,
    reviewMailCount,
    ...
    expiredUnreplied,
    breakdown: {
      validMails: `${valid} (category = DungHan)`,
      expiredMails: `${expired} (category = QuaHan)`,
